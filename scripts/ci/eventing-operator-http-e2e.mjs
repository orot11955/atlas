...
async function account(role) {
  ...
  await db.query(
    ...
    VALUES($1,$2,'Operator HTTP fixture',$3,$4,'active',now(),now(),now())`,
    [id, email, hash, role],
  );
  const login = (
  ...
  const enrollment = (
    await http('/admin/v1/auth/mfa/totp/enrollment', {
      method: 'POST',
      body: challenge,
    })
  ).data;
...
async function event(workspace, siteId, { succeeded = false, invalid = false } = {}) {
    ...
    repository.insertOutboxEvent(
      {
        ...
        payload: {
          ...
          data: {
            ...
            etag: 'a'.repeat(64),
            visibility: 'public',
            extension: 'payload-not-for-operator-view',
          },
        },
        status: 'dispatched',
        availableAt: at,
  ...
  for (let attempt = 1; attempt <= (succeeded ? 1 : 5); attempt += 1) {
    ...
    await db.transaction(async (m) => {
      const row = await repository.claimEventConsumption(
        id,
        CONSUMER_KEY,
        time,
        new Date(time.getTime() - 30_000),
        m,
      );
      assert.ok(row);
      assert.equal(
...
async function snapshot(eventId) {
  return {
    ...
    replays: await db.query(
      `SELECT r.* FROM event_consumption_replays r JOIN event_consumptions c
      ON c.id=r.consumption_id WHERE c.event_id=$1 ORDER BY r.id`,
      [eventId],
    ),
    attempts: await db.query(
      `SELECT a.* FROM event_consumption_attempts a JOIN event_consumptions c
      ON c.id=a.consumption_id WHERE c.event_id=$1 ORDER BY a.attempt_number`,
      [eventId],
    ),
    audits: await db.query(
      `SELECT * FROM audit_logs WHERE target_id=$1
      AND action='outbox.consumption-replay-requested' ORDER BY id`,
      [eventId],
    ),
  };
}
...
try {
  ...
  let viewer;
  await scenario(
    'real Password, TOTP enrollment, grant and Session authenticate OWNER and VIEWER',
    async () => {
  ...
  await db.query(
    ...
    VALUES($1,$2,'Other fixture','UTC','en',now(),now())`,
    [otherWorkspace, `operator-${otherWorkspace}`],
  );
  ...
  await scenario('VIEWER can inspect scoped receipts but cannot replay', async () => {
    const list = await http('/admin/v1/eventing/consumptions?status=dead&limit=200', {
      session: viewer,
    });
  ...
  await scenario(
    'cross-Workspace identifiers do not disclose history or permit Replay',
    async () => {
      await http(`/admin/v1/eventing/consumptions/${foreign}/history`, {
        session: owner,
        expected: 404,
      });
  ...
  await scenario(
    'authorized Replay preserves immutable Event and attempts and commits one Audit',
    async () => {
      const result = await http(route, {
        method: 'POST',
        body: replay,
        session: owner,
        expected: 202,
  ...
  await scenario(
    'identical Replay is idempotent and a conflicting Replay ID is rejected',
    async () => {
      const stable = await snapshot(dead);
      const repeated = await http(route, {
        method: 'POST',
        body: replay,
        session: owner,
        expected: 202,
      ...
      await http(route, {
        method: 'POST',
        body: { ...replay, reason: 'dependency-restored' },
        session: owner,
        expected: 409,
      });
  ...
  await scenario(
    'history and receipt HTTP views exclude payloads and raw diagnostics',
    async () => {
      const history = await http(`/admin/v1/eventing/consumptions/${dead}/history?limit=50`, {
        session: owner,
      });
      ...
      assert.doesNotMatch(
        JSON.stringify([history.data, list.data]),
        /payload-not-for-operator-view|payload_json|last_error|password_hash/u,
      );
    },
  );
  ...
  let schedule;
  await scenario(
    'authenticated schedule creation returns the exact persisted READY target',
    async () => {
      content = (
        await http('/admin/v1/contents', {
          method: 'POST',
          session: owner,
          expected: 201,
          body: {
            type: 'post',
            title: 'Operator HTTP content',
            bodyMarkdown: 'A meaningful operator test body.',
          },
        })
      ).data;
      content = (
        await http(`/admin/v1/contents/${content.id}/ready`, {
          method: 'POST',
          session: owner,
          expected: 201,
          body: {
      ...
      assignment = (
        await http(`/admin/v1/contents/${content.id}/sites`, {
          method: 'POST',
          session: owner,
          expected: 201,
      ...
      schedule = (
        await http(`/admin/v1/contents/${content.id}/sites/${assignment.id}/schedules`, {
          method: 'POST',
          session: owner,
          expected: 201,
          body: {
            action: 'publish',
            timezone: 'UTC',
            scheduledLocalAt: new Date(Date.now() + 300_000).toISOString().slice(0, 19),
          },
        })
      ).data;
      const [revision] = await db.query(
        `SELECT id,revision_number FROM content_revisions
      WHERE content_id=$1 AND kind='ready' ORDER BY revision_number DESC LIMIT 1`,
        [content.id],
      );
      assert.deepEqual(schedule.target, {
        kind: 'revision',
        revisionId: revision.id,
        revisionNumber: revision.revision_number,
      });
      assert.deepEqual(schedule.operations, { canCancel: true, canRetry: false });
      const listed = await http(`/admin/v1/publication-schedules?contentSiteId=${assignment.id}`, {
        session: owner,
      });
  ...
  await scenario(
    'schedule cancellation enforces CSRF, permission and optimistic version',
    async () => {
      ...
      await http(path, {
        method: 'POST',
        body: { version: schedule.version + 1 },
        session: owner,
        expected: 409,
      });
      ...
      const [{ count: audits }] = await db.query(
        `SELECT count(*)::int AS count FROM audit_logs
      WHERE target_id=$1 AND action='content.publication-schedule-cancelled'`,
        [schedule.id],
      );
