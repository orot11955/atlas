...
async function fixture() {
  ...
  await db.transaction(async (tx) => {
    await tx.query(
      ...
      VALUES($1,$2,'Preflight fixture','UTC','en',$3,$3)`,
    ...
    await tx.query(
      ...
      VALUES($1,$2,$3,$4,'fixture',$5,$5)`,
      [c.assignment, c.workspace, c.content, c.site, now],
    );
    ...
    await repository.insertWebhookEndpoint(
      {
        ...
        status: 'active',
        secretCiphertext: 'ciphertext-sentinel',
        secretKeyVersion: 'v1',
    ...
    await tx.query(
      ...
      [c.attempt, c.delivery, signedBody, raw, now],
    );
  });
  return c;
}
async function snapshot(c) {
  return {
    ...
    attempts: await db.query('SELECT * FROM webhook_delivery_attempts WHERE delivery_id=$1', [
      c.delivery,
    ]),
...
try {
  ...
  await db.query(
    ...
    VALUES($1,'preflight@atlas.test','Fixture','$argon2id$fixture-only','owner',$2,$2,$2)`,
    [admin, now],
  );
  ...
  await scenario(
    'pre-policy inventory counts real legacy diagnostics without disclosing or changing rows',
    async () => {
  ...
  await scenario('PostgreSQL itself rejects writes inside the collector transaction', async () => {
    ...
    try {
      await assert.rejects(
        collectEventingPreflight(reader, own.workspace),
  ...
  await scenario(
    'blocking table locks time out without changing or weakening the query',
    async () => {
  ...
  await reader.connect();
  await scenario(
    'post-policy inventory sees zero rewrites and leaves legacy targets and signed history intact',
    async () => {
      const before = await snapshot(own);
      const report = await collectEventingPreflight(reader, own.workspace);
      ...
      for (const row of Object.values(report.diagnostics))
        assert.equal(row.rewrite_candidates, '0');
  ...
  await scenario(
    'actual standalone CLI needs no key material and returns a read-only inventory',
    async () => {
      const child = spawnSync(
        process.execPath,
        ['scripts/operations/eventing-rollout-preflight.mjs', '--workspace', own.workspace],
        {
          encoding: 'utf8',
          timeout: 20_000,
          maxBuffer: 1_000_000,
