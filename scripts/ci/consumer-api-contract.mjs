...
const { ConsumerController, ConsumerReplayDto, ConsumerQueryDto } = require('./dist/eventing/consumer.controller.js');
...
for (const method of ['list', 'history', 'replay']) {
  ...
  assert.deepEqual(guards, method === 'replay'
  ...
  assert.equal(Reflect.getMetadata('atlas:admin-permission', handler),
...
for (const patch of [{replayId:'invalid'}, {expectedAttempt:'5'}, {expectedAttempt:0}, {reason:'arbitrary secret'}, {payload:{}}]) {
  assert.ok((await validate(plainToInstance(ConsumerReplayDto, {...good,...patch}),options)).length>0);
}
for (const query of [{limit:'0'},{limit:'201'},{status:'arbitrary'},{limit:5},{payload:'forbidden'}]) {
  assert.ok((await validate(plainToInstance(ConsumerQueryDto,query),options)).length>0);
}
assert.equal((await validate(plainToInstance(ConsumerQueryDto,{limit:'200',status:'dead'}),options)).length,0);
console.log('Consumer API guard metadata and DTO contract assertions passed (not an authenticated HTTP E2E).');
