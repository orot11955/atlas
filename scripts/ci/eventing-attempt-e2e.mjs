...
const options = {
  outboxBatchSize: 100,
  outboxStaleMilliseconds: 30_000,
  webhookBatchSize: 100,
  webhookStaleMilliseconds: 30_000,
  publicationBatchSize: 100,
  publicationStaleMilliseconds: 30_000,
  maximumAttempts: 3,
};
const noQueue = {
...
function webhook(
  ...
  return new WebhookDeliveryService(
    ...
    cipher,
    auditService,
    { timeoutMilliseconds: 1_000, endpointFailureThreshold: 2 },
    new FixedClock(at),
  );
}
    ...
    repository.insertOutboxEvent(
      {
        ...
        dispatchedAt: status === 'dispatched' ? startedAt : undefined,
    ...
    repository.insertWebhookEndpoint(
      {
        ...
        siteId: event.site,
        name: 'Ownership receiver',
        url: `https://hooks.example.com/${endpointId}`,
        status: 'active',
        secretCiphertext: 'fixture',
        secretKeyVersion: 'v1',
...
async function makeDelivery(attemptCount = 0) {
  ...
  if (attemptCount)
    await runner.query('UPDATE webhook_deliveries SET attempt_count = $2 WHERE id = $1', [
      deliveryId,
      attemptCount,
    ]);
...
async function snapshot(context) {
  const [event] = await runner.query('SELECT * FROM outbox_events WHERE id = $1', [
    context.eventId,
  ]);
  const consumptions = await runner.query(
    'SELECT * FROM event_consumptions WHERE event_id = $1 ORDER BY id',
    [context.eventId],
  );
  const deliveries = await runner.query(
    'SELECT * FROM webhook_deliveries WHERE event_id = $1 ORDER BY id',
    [context.eventId],
  );
  const attempts = await runner.query(
    `SELECT attempt.* FROM webhook_delivery_attempts attempt
    JOIN webhook_deliveries delivery ON delivery.id = attempt.delivery_id WHERE delivery.event_id = $1 ORDER BY attempt.id`,
    [context.eventId],
  );
  const endpoints = await runner.query(
    'SELECT * FROM webhook_endpoints WHERE workspace_id = $1 ORDER BY id',
    [context.workspace],
  );
  const logs = await runner.query('SELECT * FROM audit_logs WHERE workspace_id = $1 ORDER BY id', [
    context.workspace,
  ]);
  return { event, consumptions, deliveries, attempts, endpoints, logs };
}
async function claimEvent(connection, at = startedAt) {
  const events = await transaction(connection, (manager) =>
    repository.claimAvailableOutboxEvents(at, new Date(at.getTime() - 30_000), 100, manager),
  );
...
async function finishEvent(connection, owner) {
  return transaction(connection, (manager) =>
    repository.markOutboxEventDispatched(owner, recoveredAt, manager),
  );
}
...
async function recoverWebhook(connection) {
  return transaction(connection, (manager) =>
    repository.recoverStaleWebhookDeliveries(staleBefore, recoveredAt, manager),
  );
}
async function finishWebhook(connection, owner, status = 'succeeded', at = recoveredAt) {
  return transaction(connection, (manager) =>
    repository.finishWebhookDeliveryExecution(
      owner,
      {
        status,
        finishedAt: at,
        endpointFailureThreshold: 2,
...
try {
  ...
  await transaction(runner, async () => {
    ...
    await runner.query(
      ...
      '$argon2id$fixture-only', 'owner', now(), now(), now())`,
      [admin],
    );
  });
  console.log(
  ...
  await scenario('outbox claims skip a row locked by another worker', async () => {
    ...
    const events = await repository.claimAvailableOutboxEvents(
      startedAt,
      staleBefore,
      100,
      a.manager,
    ...
    assert.equal(events.length, 1);
    const others = await transaction(b, (manager) =>
      repository.claimAvailableOutboxEvents(startedAt, staleBefore, 100, manager),
    );
  ...
  for (const outcome of ['success', 'failure']) {
    await scenario(`late outbox enqueue ${outcome} cannot overwrite a newer dispatch`, async () => {
      ...
      else release.reject(new Error('old queue error'));
      await bounded(old);
  ...
  await scenario('outbox owner scope and monotonic manual retry fence old finalizers', async () => {
    ...
    await transaction(a, (manager) =>
      repository.rescheduleOutboxEvent(
        owner,
        recoveredAt,
        'dead fixture',
        true,
    ...
    assert.equal(
      await transaction(b, (manager) =>
        repository.retryDeadOutboxEvent(context.workspace, context.eventId, recoveredAt, manager),
      ),
      true,
    );
    ...
    assert.equal(
      await transaction(a, (manager) =>
        repository.rescheduleOutboxEvent(owner, recoveredAt, 'late', true, recoveredAt, manager),
      ),
      false,
    );
    assert.deepEqual(await snapshot(context), current);
  ...
  await scenario(
    'late Consumer completion and failure cannot change a succeeded receipt',
    async () => {
      const context = await makeEvent();
      const first = await transaction(a, (manager) =>
        repository.claimEventConsumption(
          context.eventId,
          'atlas.eventing.v1',
          startedAt,
          new Date(0),
      ...
      for (const status of ['succeeded', 'failed']) {
        assert.equal(
          await transaction(a, (manager) =>
            repository.completeEventConsumption(
              owner,
              status,
              { processedAt: recoveredAt, errorMessage: 'late', result: { overwritten: true } },
              manager,
            ),
          ),
          false,
  ...
  await scenario(
    'Consumer effect transaction holds ownership against an actual competing claim',
    async () => {
      const context = await makeEndpoint(await makeEvent());
      const record = await transaction(a, (manager) =>
        repository.claimEventConsumption(
          context.eventId,
          'atlas.eventing.v1',
          startedAt,
      ...
      const competing = transaction(b, (manager) =>
        repository.claimEventConsumption(
          context.eventId,
          'atlas.eventing.v1',
          recoveredAt,
      ...
      assert.equal(
        await repository.completeEventConsumption(
          owner,
          'succeeded',
  ...
  await scenario(
    'post-commit queue failure preserves receipt and pending work is rediscovered',
    async () => {
      const context = await makeEndpoint(await makeEvent());
      let submitted = 0;
      await assert.rejects(
        consumer(a, {
          ...
          enqueueWebhookDelivery: async () => {
            assert.equal(a.isTransactionActive, false);
            submitted += 1;
            throw new Error('queue offline after DB commit');
          },
        }).consume(context.eventId),
        /queue offline/u,
      );
      ...
      await relay(
        ...
        {
          ...noQueue,
          enqueueWebhookDelivery: async (input) => {
            assert.equal(b.isTransactionActive, false);
            notified.push(input.deliveryId);
          },
        },
        recoveredAt,
      ).recoverDueWork();
  ...
  await scenario(
    'Consumer Audit failure rolls back inserted deliveries and success receipt together',
    async () => {
      const context = await makeEndpoint(await makeEvent());
      const failingAudit = new AuditService(
        {
          insert: async (record, manager) => {
            ...
            if (record.action === 'outbox.event-consumed')
              throw new Error('injected consumption Audit failure');
          },
        },
        clock,
      );
  ...
  await scenario(
    'Webhook owner fields are independently checked and duplicate finalization is inert',
    async () => {
  ...
  for (const outcome of ['success', 'retryable-failure', 'terminal-failure']) {
    await scenario(
      `late Webhook ${outcome} preserves new Delivery, Attempt, Endpoint and Audit`,
      async () => {
        const count = outcome === 'terminal-failure' ? 5 : 0;
        const context = await makeDelivery(count);
        const entered = deferred();
        const release = deferred();
        const old = webhook(a, async () => {
          entered.resolve();
          return release.promise;
        }).deliver(context.deliveryId, count + 1);
        void old.catch(() => undefined);
        await bounded(entered.promise);
  ...
  await scenario(
    'new endpoint configuration is not disabled by an old configuration response',
    async () => {
      const context = await makeDelivery(5);
      const execution = await claimWebhook(a, context.deliveryId, 6);
      await transaction(b, (manager) =>
        repository.updateWebhookEndpoint(
          context.workspace,
          context.endpointId,
          {
            expectedVersion: 1,
            nextVersion: 2,
            name: 'Reconfigured',
            url: `https://hooks.example.com/${context.endpointId}/new`,
            subscribedEvents: ['content.published'],
  ...
  await scenario(
    'only a current terminal Webhook result changes the Endpoint failure policy once',
    async () => {
      const context = await makeDelivery(5);
  ...
  await scenario(
    'Webhook recovery skips locked finalization and never broadly updates skipped rows',
    async () => {
      const context = await makeDelivery();
      const owner = ownerOfWebhook(await claimWebhook(a, context.deliveryId));
      await a.startTransaction();
      assert.equal(
        await repository.finishWebhookDeliveryExecution(
          owner,
          {
            status: 'succeeded',
  ...
  await scenario(
    'Webhook finalization waiting for recovery rechecks ownership after the actual lock wait',
    async () => {
      const context = await makeDelivery();
      const owner = ownerOfWebhook(await claimWebhook(a, context.deliveryId));
      await a.startTransaction();
      assert.equal(
        await repository.recoverStaleWebhookDeliveries(staleBefore, recoveredAt, a.manager),
        1,
      );
  ...
  await scenario(
    'Webhook retries respect persisted due time and manual retry never resets attempt count',
    async () => {
      const context = await makeDelivery();
      const old = ownerOfWebhook(await claimWebhook(a, context.deliveryId));
      ...
      assert.equal(
        await transaction(b, (manager) =>
          repository.resetWebhookDeliveryForRetry(
            context.workspace,
            context.deliveryId,
            recoveredAt,
            manager,
  ...
  await scenario(
    'Webhook Audit persistence failure rolls back the whole result and is not an HTTP retry',
    async () => {
      const context = await makeDelivery();
      const failingAudit = new AuditService(
        {
          insert: async (record, manager) => {
            await auditRepository.insert(record, manager);
            throw new Error('injected Webhook Audit failure');
          },
        },
        clock,
      );
      await assert.rejects(
        webhook(a, undefined, failingAudit).deliver(context.deliveryId, 1),
        /injected Webhook/u,
      );
  ...
  await scenario(
    'all Eventing ownership mutation entry points reject an autocommit manager',
    async () => {
      const context = await makeDelivery();
      const eventOwner = {
        eventId: context.eventId,
      ...
      await assert.rejects(
        repository.claimAvailableOutboxEvents(startedAt, staleBefore, 1, a.manager),
        /active transaction/u,
      );
      await assert.rejects(
        repository.markOutboxEventDispatched(eventOwner, recoveredAt, a.manager),
        /active transaction/u,
      );
      await assert.rejects(
        repository.rescheduleOutboxEvent(
      ...
      await assert.rejects(
        repository.claimEventConsumption(context.eventId, 'c', startedAt, staleBefore, a.manager),
        /active transaction/u,
      );
      await assert.rejects(
        repository.startWebhookDeliveryAttempt(
      ...
      await assert.rejects(
        repository.recoverStaleWebhookDeliveries(staleBefore, recoveredAt, a.manager),
        /active transaction/u,
      );
    },
  );
