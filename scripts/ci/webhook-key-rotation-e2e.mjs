...
async function fixture() {
  ...
  await tx(control, async (m) => {
    ...
    for (const [i, id] of c.endpoints.entries()) {
      ...
      if (i === 1) {
        await m.query(
          `UPDATE webhook_endpoints SET status='disabled',disabled_at=now() WHERE id=$1`,
          [id],
        );
      }
    }
...
async function snapshot(c) {
  return {
    rows: await control.query('SELECT * FROM webhook_endpoints WHERE workspace_id=$1 ORDER BY id', [
      c.workspace,
    ]),
    logs: await control.query('SELECT * FROM audit_logs WHERE workspace_id=$1 ORDER BY id', [
      c.workspace,
    ]),
  };
}
try {
  ...
  await scenario(
    'batch preserves signatures and every non-storage endpoint field including disabled state',
    async () => {
      ...
      for (const [i, row] of after.rows.entries()) {
        ...
        assert.deepEqual(
          {
            ...row,
            secret_ciphertext: old.secret_ciphertext,
            secret_key_version: old.secret_key_version,
          },
          old,
        );
      }
      assert.equal(
        (await scoped(c.workspace, () => service(a).reencryptBatch(c.workspace, 'v2'))).changed,
        0,
      );
  ...
  await scenario('Audit insertion failure rolls back the entire rewrite', async () => {
    ...
    await assert.rejects(
      scoped(c.workspace, () => service(a, failure).reencryptBatch(c.workspace, 'v2')),
      /Injected/u,
    );
    assert.deepEqual(await snapshot(c), before);
  });
  await scenario(
    'tampered stored ciphertext fails without modifying earlier rows or Audit',
    async () => {
      const c = await fixture();
      const clean = await snapshot(c);
      const corruptId = clean.rows[1].id;
      await control.query('UPDATE webhook_endpoints SET secret_ciphertext=$2 WHERE id=$1', [
        corruptId,
        'w1.invalid.invalid.invalid',
      ]);
      const before = await snapshot(c);
      ...
      await control.query('UPDATE webhook_endpoints SET secret_ciphertext=$2 WHERE id=$1', [
        corruptId,
        clean.rows[1].secret_ciphertext,
      ]);
    },
  );
  await scenario(
    'a separate worker skips locked rows instead of overwriting or claiming completion',
    async () => {
      const c = await fixture();
      await a.startTransaction();
      try {
        await a.query('SELECT id FROM webhook_endpoints WHERE workspace_id=$1 FOR UPDATE', [
          c.workspace,
        ]);
        assert.equal(
          (await scoped(c.workspace, () => service(b).reencryptBatch(c.workspace, 'v2'))).changed,
          0,
        );
  ...
  await scenario('stale storage CAS cannot undo an ordinary signing-secret rotation', async () => {
    ...
    assert.equal(
      await tx(b, (m) =>
        repository.replaceCiphertext(
          ...
          current.encrypt(secret).encryptedValue,
          'v2',
          m,
        ),
      ),
    ...
    assert.equal(
      current.decrypt(after.secret_ciphertext, after.secret_key_version),
      replacementSecret,
    );
  });
  await scenario(
    'coverage and retirement checks include other Workspaces and disabled endpoints',
    async () => {
      await fixture();
  ...
  await scenario('unscoped operation and wrong active version cannot write', async () => {
    ...
    await assert.rejects(
      scoped(c.workspace, () => service(a).reencryptBatch(c.workspace, 'wrong')),
    );
    assert.deepEqual(await snapshot(c), before);
  });
  await scenario(
    'after all stored references are rewritten only the new key is needed for DB rows',
    async () => {
      ...
      for (const c of contexts) {
        for (const row of (await snapshot(c)).rows) {
          assert.equal(
            newOnly.decrypt(row.secret_ciphertext, 'v2'),
            current.decrypt(row.secret_ciphertext, 'v2'),
          );
        }
      }
    },
