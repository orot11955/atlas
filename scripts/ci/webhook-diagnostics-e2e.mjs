...
const { WebhookTransportError, WEBHOOK_RESPONSE_OMITTED } = require(
  resolve(root, 'packages/server/dist/modules/eventing/domain/webhook-diagnostics.js'),
);
...
function service(connection, send, auditService = audit, at = now) {
  return new WebhookDeliveryService(
    ...
    { decrypt: () => 's'.repeat(48) },
    auditService,
    { timeoutMilliseconds: 100, endpointFailureThreshold: 3 },
    new FixedClock(at),
  );
}
async function fixture() {
  ...
  await tx(control, async (m) => {
    ...
    await repository.insertWebhookDeliveryIfAbsent(
      {
        id: c.delivery,
        workspaceId: c.workspace,
...
async function snapshot(c) {
  const [delivery] = await control.query('SELECT * FROM webhook_deliveries WHERE id=$1', [
    c.delivery,
  ]);
  const attempts = await control.query(
    'SELECT * FROM webhook_delivery_attempts WHERE delivery_id=$1 ORDER BY attempt_number',
    [c.delivery],
  );
  const logs = await control.query('SELECT * FROM audit_logs WHERE workspace_id=$1 ORDER BY id', [
    c.workspace,
  ]);
...
async function blockedWrite(table, field, id, value) {
  await assert.rejects(
    tx(control, (m) => m.query(`UPDATE ${table} SET ${field}=$2 WHERE id=$1`, [id, value])),
...
try {
  ...
  const migrations = files.map((file) => {
    const types = Object.values(require(resolve(directory, file))).filter(
  ...
  const index = migrations.findIndex(
    (m) => m.name === 'EnforceWebhookDiagnosticPolicy1788696000000',
  );
  assert.ok(index >= 0);
  ...
  await control.query(
    ...
    VALUES($1,'webhook@atlas.test','Fixture','$argon2id$fixture-only','owner',$2,$2,$2)`,
    [admin, now],
  );
  const legacy = await fixture();
  const legacyAttempt = createUuidV7();
  await control.query(
    `UPDATE webhook_deliveries SET status='succeeded',attempt_count=1,completed_at=$2,
    last_response_status=200,last_response_excerpt=$3,last_error=$3 WHERE id=$1`,
    [legacy.delivery, now, secret],
  );
  await control.query(
    `INSERT INTO webhook_delivery_attempts(id,delivery_id,attempt_number,status,request_body,
    response_status,response_body_excerpt,error_message,requested_at,completed_at)
    VALUES($1,$2,1,'succeeded',$3,200,$4,$4,$5,$5)`,
    [legacyAttempt, legacy.delivery, signedBody, secret, now],
  );
  const before = await snapshot(legacy);
  for (const migration of migrations.slice(index)) await tx(control, () => migration.up(control));
  console.log(
    `Applied ${migrations.length} actual migrations with pre-policy diagnostic fixtures.`,
  );

  await scenario(
    'legacy cleanup preserves all non-diagnostic fields and signed request bytes',
    async () => {
  ...
  await scenario(
    'database boundary refuses raw response and exception text in both tables',
    async () => {
  ...
  await scenario(
    'success sanitizes alternate Sender output and duplicate execution is inert',
    async () => {
  ...
  await scenario(
    'arbitrary errors, Deadline and invalid Sender statuses use fixed diagnostics',
    async () => {
      for (const [send, expected] of [
        ...
        const c = await fixture();
        await service(a, send).deliver(c.delivery, 1);
        const after = await snapshot(c);
        assert.equal(after.delivery.status, 'retry_scheduled');
  ...
  await scenario(
    'late response cannot replace a recovered owner result or append Audit',
    async () => {
      const c = await fixture();
      let enter;
      let release;
  ...
  await scenario(
    'Audit failure rolls back the result without becoming a second HTTP failure',
    async () => {
      const c = await fixture();
      const failing = new AuditService(
        {
          insert: async (record, m) => {
            ...
            throw new Error('Injected diagnostic Audit failure.');
          },
        },
        clock,
      );
      await assert.rejects(
        service(a, async () => ({ status: 200, bodyExcerpt: secret }), failing).deliver(
          c.delivery,
          1,
        ),
        /Injected/u,
      ...
      const after = await snapshot(c);
      assert.equal(after.delivery.status, 'processing');
      assert.equal(after.attempts[0].status, 'processing');
      assert.equal(after.delivery.last_response_excerpt, null);
      assert.equal(after.logs.length, 0);
    },
  );
  await scenario(
    'existing administration read model cannot expose old diagnostic content',
    async () => {
