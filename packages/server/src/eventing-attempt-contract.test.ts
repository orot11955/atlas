...
function manager(result: unknown, active = true): EntityManager {
  return {
    ...
    query: async () => (typeof result === 'function' ? result() : result),
...
for (const [name, operation] of [
  [
    'outbox',
    (tx: EntityManager) => finishOutboxAttempt(outbox, { status: 'dispatched', at: now }, tx),
  ],
  ['consumption-lock', (tx: EntityManager) => lockConsumption(consumption, tx)],
  [
    'consumption-finish',
    (tx: EntityManager) => finishConsumption(consumption, 'succeeded', { processedAt: now }, tx),
  ],
...
test('outbox owner is mandatory and zero changed rows is a stale no-op', async () => {
  await assert.rejects(
    finishOutboxAttempt(undefined as never, { status: 'dispatched', at: now }, manager([])),
    /owner/u,
  );
  await assert.rejects(
    finishOutboxAttempt(
      { ...outbox, attemptNumber: 0 },
      { status: 'dispatched', at: now },
      manager([]),
    ),
    /owner/u,
  );
  assert.equal(
    await finishOutboxAttempt(outbox, { status: 'dispatched', at: now }, manager([[], 0])),
    false,
  );
  assert.equal(
    await finishOutboxAttempt(
      outbox,
      { status: 'dispatched', at: now },
      manager([[{ id: 'event' }], 1]),
    ),
...
test('consumption owner includes event, workspace and consumer identity', async () => {
  ...
  await assert.rejects(
    lockConsumption({ ...consumption, attemptNumber: NaN }, manager([])),
    /owner/u,
  );
});
...
test('webhook completion rejects missing identity and impossible retry policies', async () => {
  for (const owner of [
    undefined,
    { ...webhook, attemptId: '' },
    { ...webhook, endpointVersion: 0 },
  ]) {
    await assert.rejects(finishWebhookExecution(owner as never, completion, manager([])));
  }
  await assert.rejects(
    finishWebhookExecution(webhook, { ...completion, status: 'retry_scheduled' }, manager([])),
  );
  await assert.rejects(
    finishWebhookExecution(webhook, { ...completion, nextRetryAt: now }, manager([])),
  );
});
