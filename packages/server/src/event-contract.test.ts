...
function event(eventType = 'content.published'): OutboxEventRecord {
  ...
  return {
    id, workspaceId, siteId: retry ? undefined : siteId, aggregateId,
    aggregateType: schedule ? 'publication-schedule' : retry ? 'webhook-delivery' : 'content-publication',
    eventType, schemaVersion: 1, status: 'dispatched', availableAt: now,
    attemptCount: 1, createdAt: now, updatedAt: now,
    payload: {
      eventId: id, eventType, occurredAt: now.toISOString(), workspaceId,
      siteId: retry ? null : siteId, aggregateId, schemaVersion: 1,
      data: schedule || retry
        ? { [retry ? 'deliveryId' : 'scheduleId']: aggregateId, attemptNumber: 1, availableAt: now.toISOString() }
        : { publicationId: aggregateId, contentId: createUuidV7(), contentSiteId: createUuidV7(),
            revisionId: createUuidV7(), revisionNumber: 1, slug: 'article', etag: 'e'.repeat(64),
...
for (const eventType of Object.keys(EVENT_CONTRACT_REGISTRY)) {
  test(`registered v1 ${eventType} routes without modifying its payload`, () => {
    ...
    assert.equal(route.kind, EVENT_CONTRACT_REGISTRY[eventType as keyof typeof EVENT_CONTRACT_REGISTRY].handler);
...
test('registry has five frozen descriptors with no prototype fallback', () => {
  ...
  for (const descriptor of Object.values(EVENT_CONTRACT_REGISTRY)) assert.ok(Object.isFrozen(descriptor));
});

const withData = (r: OutboxEventRecord, patch: Record<string, unknown>) =>
  ({ ...r, payload: { ...r.payload, data: { ...r.payload.data, ...patch } } });
const mutations: ReadonlyArray<readonly [string, (r: OutboxEventRecord) => unknown]> = [
  ...
  ['bad date rollover', (r) => ({ ...r, payload: { ...r.payload, occurredAt: '2030-02-30T00:00:00.000Z' } })],
  ...
  ['cyclic extension', (r) => { const x: Record<string, unknown> = {}; x.self = x; return withData(r, { extension: x }); }],
  ['deep extension', (r) => { let x: unknown = 1; for (let i = 0; i < 33; i++) x = [x]; return withData(r, { extension: x }); }],
];
for (const [name, mutate] of mutations) {
  test(`invalid event fails closed: ${name}`, () => {
    const value = mutate(event());
    assert.throws(() => resolveEventContract(value as OutboxEventRecord), (error: unknown) => {
      assert.ok(error instanceof EventContractError);
      assert.equal(error.code, 'VALIDATION_FAILED');
      assert.doesNotMatch(JSON.stringify({ message: error.message, details: error.details }), /sensitive-marker/);
      return true;
    });
  });
}
for (const name of ['publication.schedule.requested', 'webhook.delivery.retry-requested']) {
  test(`${name} rejects invalid queue arguments rather than coercing them`, () => {
    const r = event(name);
    for (const patch of [{ attemptNumber: '1' }, { attemptNumber: 0 }, { availableAt: 'tomorrow' },
...
test('valid bounded extension data and rollback metadata remain unchanged', () => {
  const r = withData(event(), { sourcePublicationId: createUuidV7(), replacedPublicationId: null,
