...
export const EVENT_CONTRACT_REGISTRY = Object.freeze({
  'content.published': Object.freeze({ schemaVersion: 1, aggregateType: 'content-publication', handler: 'publication', siteRequired: true }),
  'content.unpublished': Object.freeze({ schemaVersion: 1, aggregateType: 'content-publication', handler: 'publication', siteRequired: true }),
  'publication.schedule.requested': Object.freeze({ schemaVersion: 1, aggregateType: 'publication-schedule', handler: 'schedule', siteRequired: true }),
  'publication.schedule.retry-requested': Object.freeze({ schemaVersion: 1, aggregateType: 'publication-schedule', handler: 'schedule', siteRequired: true }),
  'webhook.delivery.retry-requested': Object.freeze({ schemaVersion: 1, aggregateType: 'webhook-delivery', handler: 'webhook-retry', siteRequired: false }),
...
export class EventContractError extends DomainError {
  public constructor(public readonly reason: EventContractReason) {
    super({ code: ErrorCode.VALIDATION_FAILED, message: `Outbox Event contract rejected: ${reason}.`, details: { reason } });
  }
}
...
export type RegisteredEventRoute =
  ...
  | Readonly<{ kind: 'webhook-retry'; deliveryId: string; attemptNumber: number; availableAt: Date }>;
...
export function resolveEventContract(event: Readonly<OutboxEventRecord>): RegisteredEventRoute {
  ...
  if (!isObject(payload) || !isUuid(event.id) || !isUuid(event.workspaceId) ||
      !isUuid(event.aggregateId) || (event.siteId != null && !isUuid(event.siteId)) ||
      (descriptor.siteRequired && !isUuid(event.siteId)) || !isTimestamp(payload.occurredAt)) {
    throw new EventContractError('invalid-envelope');
  }
  if (payload.eventId !== event.id || payload.workspaceId !== event.workspaceId ||
      payload.siteId !== (event.siteId ?? null) || payload.aggregateId !== event.aggregateId ||
      payload.eventType !== event.eventType || payload.schemaVersion !== event.schemaVersion) {
  ...
  if (descriptor.handler === 'publication') {
    ...
    if (!isPositiveInteger(data.revisionNumber) || !isText(data.slug, 512) || !isText(data.etag, 256)) {
      throw new EventContractError('invalid-event-data');
    }
    if (event.eventType === 'content.published' &&
        (typeof data.visibility !== 'string' || !['public', 'unlisted', 'private'].includes(data.visibility))) {
      throw new EventContractError('invalid-event-data');
    }
    if ((data.replacedPublicationId != null && !isUuid(data.replacedPublicationId)) ||
        (data.sourcePublicationId !== undefined && !isUuid(data.sourcePublicationId)) ||
        (data.scheduled !== undefined && typeof data.scheduled !== 'boolean')) {
      throw new EventContractError('invalid-event-data');
    }
    return Object.freeze({ kind: 'publication', eventType: event.eventType as WebhookEventType, siteId: event.siteId! });
  }
