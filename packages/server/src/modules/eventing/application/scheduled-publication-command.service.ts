 ...
 */
export type ScheduledPublicationFactory<TTransaction> = (
  ...
  public async executeScheduled(
    ...
    return this.transactions.run(async (transaction) => {
      ...
      if (
        ...
        schedule.attemptCount !== owner.attemptNumber ||
        schedule.version !== owner.version
      ) {
      ...
      if (
        ...
        context.actorId !== schedule.requestedByAdminAccountId ||
        context.workspaceId !== schedule.workspaceId ||
        context.siteId !== schedule.siteId
      ) {
      ...
      if (previous) {
        if (
          previous.contentId !== schedule.contentId ||
          previous.contentSiteId !== schedule.contentSiteId ||
          previous.siteId !== schedule.siteId ||
          !sameTarget(previous.target, target)
        ) {
          throw new Error(
            'Publication schedule effect receipt does not match its immutable definition.',
          );
        }
      ...
      if (!contentSite || contentSite.siteId !== schedule.siteId) {
        throw new DomainError({
          ...
          message: 'Scheduled Content Site was not found.',
        });
      }
      ...
      if (target.action === 'publish') {
        ...
        if (
          result.publication.revisionId !== target.revisionId ||
        ...
        if (
          !expected ||
          expected.contentId !== schedule.contentId ||
          expected.siteId !== schedule.siteId
        ) {
          throw new DomainError({
            code: ErrorCode.NOT_FOUND,
            message: 'Scheduled Publication target was not found.',
          });
        }
        ...
        if (expected.status === 'withdrawn' || expected.status === 'superseded') {
          ...
          const withdrawn = await command.withdraw(
            schedule.workspaceId,
            schedule.contentId,
            schedule.contentSiteId,
          );
          if (withdrawn.id !== expected.id)
            throw new Error('Withdrawal changed a different Publication.');
          outcome = 'withdrawn';
        }
      }
