...
export async function finishOutboxAttempt(
  ...
  input: Readonly<{
    status: 'dispatched' | 'pending' | 'dead';
    at: Date;
    availableAt?: Date;
    error?: string;
  }>,
  ...
  return changed(
    await transaction.query(
     ...
     SET status = $4::varchar, claimed_at = NULL,
         dispatched_at = CASE WHEN $4::varchar = 'dispatched' THEN $5::timestamptz ELSE NULL END,
...
export async function finishWebhookExecution(
  ...
  if (
    !Number.isSafeInteger(owner.endpointVersion) ||
    owner.endpointVersion < 1 ||
    !Number.isSafeInteger(input.endpointFailureThreshold) ||
    input.endpointFailureThreshold < 1 ||
    ...
    (input.status === 'retry_scheduled' &&
      (!(input.nextRetryAt instanceof Date) ||
        ...
        input.nextRetryAt < input.finishedAt)) ||
    ...
    await transaction.query(
     ...
     WHERE id = $1 AND workspace_id = $2 AND endpoint_id = $3 AND attempt_count = $4
      ...
      [
        ...
        input.responseBodyExcerpt ?? null,
        input.errorMessage ?? null,
        input.status === 'retry_scheduled' ? null : input.finishedAt,
        input.finishedAt,
      ],
    ),
  );
  const attemptChanged = changed(
    await transaction.query(
  ...
  if (!deliveryChanged || !attemptChanged)
    throw new Error('Locked Webhook ownership changed unexpectedly.');
      ...
      await transaction.query(
       ...
       WHERE id = $1 AND workspace_id = $2 AND version = $3 AND status = 'active' RETURNING id`,
        [
          owner.endpointId,
          owner.workspaceId,
          owner.endpointVersion,
          input.endpointFailureThreshold,
          input.finishedAt,
        ],
      ),
    );
...
export async function recoverWebhookExecutions(
  ...
  requireEventingTransaction(transaction);
  const rows = unwrapTypeOrmMutationRows<{ id: string; attempt_count: number }>(
    await transaction.query(
      `WITH stale AS (
