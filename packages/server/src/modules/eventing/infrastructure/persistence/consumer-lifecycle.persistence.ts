...
import {
  CONSUMER_ATTEMPTS_PER_CYCLE,
  CONSUMER_KEY,
  CONSUMER_NOTIFICATION_LEASE_MS,
  retryDelay,
  type ConsumerState,
  type ConsumptionCompletion,
  type ConsumptionNotification,
  type ConsumptionReplayInput,
  type ConsumptionReplayResult,
...
function uuid(value: unknown): asserts value is string {
  if (typeof value !== 'string' || !isUuidV7(value))
    throw new Error('A valid Consumer identity is required.');
}
function instant(at: Date): void {
  if (!(at instanceof Date) || !Number.isFinite(at.getTime()))
    throw new Error('A valid Consumer timestamp is required.');
}
function ownerIdentity(owner: Readonly<EventConsumptionOwner>): void {
  if (!owner) throw new Error('A Consumer owner is required.');
  uuid(owner.consumptionId);
  uuid(owner.eventId);
  uuid(owner.workspaceId);
  key(owner.consumerKey);
  if (!Number.isSafeInteger(owner.attemptNumber) || owner.attemptNumber < 1)
    throw new Error('A positive Consumer attempt is required.');
}
function key(value: string): void {
  if (
    ...
    throw new Error('A valid Consumer key is required.');
}
function limitSize(limit: number): void {
  if (!Number.isSafeInteger(limit) || limit < 1 || limit > 200)
    throw new Error('Consumer batch limit must be 1..200.');
}
function singleton(result: unknown): boolean {
  const rows = unwrapTypeOrmMutationRows<{ id: string }>(result);
  if (rows.length > 1 || rows.some((row) => typeof row.id !== 'string'))
    throw new Error('Invalid Consumer mutation result.');
...
async function history(
  ...
  await tx.query(
    `INSERT INTO event_consumption_attempts
...
async function expire(row: Row, at: Date, tx: EntityManager): Promise<void> {
  ...
  await tx.query(
    `UPDATE event_consumptions SET status=$2, processed_at=$3, next_attempt_at=$4,
    result_json=NULL, failure_code='claim-expired', last_error='Consumer claim expired.', notify_after=$3, updated_at=$3
    WHERE id=$1`,
    [row.id, dead ? 'dead' : 'failed', at, dead ? null : at],
  );
  row.status = dead ? 'dead' : 'failed';
  row.processed_at = at;
  row.next_attempt_at = dead ? null : at;
}
...
export async function claimConsumption(
  ...
  await tx.query(
    ...
    [createUuidV7(at.getTime()), consumerKey, eventId, at],
  );
  const rows: Row[] = await tx.query(
    `SELECT c.*,e.workspace_id FROM event_consumptions c
    JOIN outbox_events e ON e.id=c.event_id WHERE c.consumer_key=$1 AND c.event_id=$2
    AND e.status IN ('processing','dispatched') FOR UPDATE OF c`,
    [consumerKey, eventId],
  );
  const row = rows[0];
  ...
  if (!row.next_attempt_at || row.next_attempt_at > at || row.attempt_count >= row.attempt_limit)
    return undefined;
  const result = await tx.query(
    `UPDATE event_consumptions SET status='processing',attempt_count=attempt_count+1,
    claimed_at=$2,processed_at=NULL,next_attempt_at=NULL,result_json=NULL,last_error=NULL,failure_code=NULL,
    notify_after=$2,updated_at=$2 WHERE id=$1 RETURNING *`,
    [row.id, at],
  );
...
export async function finishDurableConsumption(
  ...
  const terminal =
    status === 'failed' &&
    (input.permanentFailure === true || row.attempt_count >= row.attempt_limit);
  const next =
    status === 'failed' && !terminal
      ? new Date(
          input.processedAt.getTime() + retryDelay(row.attempt_count, row.cycle_start_attempt),
        )
      : null;
  const code =
    status === 'succeeded'
  ...
  const changed = singleton(
    ...
    result_json=$5,last_error=$6,failure_code=$7,notify_after=$3,updated_at=$3 WHERE id=$1 RETURNING id`,
      [
        ...
        input.processedAt,
        next,
        status === 'succeeded' ? (input.result ?? null) : null,
        status === 'succeeded'
          ? null
          : 'Consumer processing failed; inspect failure code and correlated logs.',
        code,
      ],
    ),
  );
...
export async function reserveNotifications(
  now: Date,
  staleBefore: Date,
  limit: number,
  tx: EntityManager,
  ...
  const missing: { id: string }[] = await tx.query(
    ...
    ORDER BY e.dispatched_at,e.id FOR UPDATE OF e SKIP LOCKED LIMIT $3`,
    [missedBefore, CONSUMER_KEY, limit],
  );
  ...
  const rows: Row[] = await tx.query(
    ...
    ORDER BY COALESCE(c.next_attempt_at,c.claimed_at),c.id FOR UPDATE OF c SKIP LOCKED LIMIT $4`,
    [CONSUMER_KEY, now, staleBefore, limit],
  );
  ...
  for (const row of rows) {
    ...
    const result = unwrapTypeOrmMutationRows<{ id: string; notification_version: number }>(
      await tx.query(
    ...
    notifications.push({
      ...
      notificationVersion: result[0]!.notification_version,
      availableAt: now,
    });
  }
...
export async function listConsumptions(
  ...
  if (
    status !== undefined &&
    !['pending', 'processing', 'failed', 'dead', 'succeeded'].includes(status)
  )
    throw new Error('Invalid Consumer status.');
  const rows: Row[] = await tx.query(
    `SELECT c.* FROM event_consumptions c JOIN outbox_events e ON e.id=c.event_id
    WHERE e.workspace_id=$1 AND c.consumer_key=$2 AND ($3::text IS NULL OR c.status=$3)
    ORDER BY c.updated_at DESC,c.id DESC LIMIT $4`,
    [workspaceId, CONSUMER_KEY, status ?? null, limit],
  );
  return rows.map((r) => ({
    id: r.id,
    eventId: r.event_id,
    ...
    cycleStartAttempt: r.cycle_start_attempt,
...
export async function readHistory(
  ...
  const rows: { id: string }[] = await tx.query(
    `SELECT c.id FROM event_consumptions c JOIN outbox_events e ON e.id=c.event_id
    WHERE e.workspace_id=$1 AND c.event_id=$2 AND c.consumer_key=$3`,
    [workspaceId, eventId, CONSUMER_KEY],
  );
  if (!rows[0])
    throw new DomainError({
      code: ErrorCode.NOT_FOUND,
      message: 'Consumer receipt was not found.',
    });
  const attempts: Record<string, unknown>[] = await tx.query(
    `SELECT attempt_number,cycle_start_attempt,outcome,failure_code,started_at,finished_at
    FROM event_consumption_attempts WHERE consumption_id=$1 ORDER BY attempt_number DESC LIMIT $2`,
    [rows[0].id, limit],
  );
  const replays: Record<string, unknown>[] = await tx.query(
    `SELECT id,previous_attempt,previous_limit,next_limit,reason,created_at
    FROM event_consumption_replays WHERE consumption_id=$1 ORDER BY created_at DESC,id DESC LIMIT $2`,
    [rows[0].id, limit],
  );
  return { attempts, replays };
}

export async function replayConsumption(
  workspaceId: string,
  actorId: string,
  ...
  if (
    ...
    !['dependency-restored', 'handler-upgraded', 'operator-reviewed'].includes(input.reason)
  )
    throw new DomainError({
      code: ErrorCode.VALIDATION_FAILED,
      message: 'Invalid Consumer replay request.',
    });
  const rows: Row[] = await tx.query(
    `SELECT c.*,e.workspace_id FROM event_consumptions c JOIN outbox_events e ON e.id=c.event_id
    WHERE c.event_id=$1 AND c.consumer_key=$2 AND e.workspace_id=$3 FOR UPDATE OF c`,
    [input.eventId, CONSUMER_KEY, workspaceId],
  );
  const row = rows[0];
  if (!row)
    throw new DomainError({
  ...
  const prior = await tx.query(`SELECT * FROM event_consumption_replays WHERE id=$1`, [
    input.replayId,
  ]);
  if (prior[0]) {
    const p = prior[0];
    if (
      p.consumption_id !== row.id ||
      p.previous_attempt !== input.expectedAttempt ||
      p.reason !== input.reason ||
      p.requested_by_admin_account_id !== actorId
    )
      throw new DomainError({
        code: ErrorCode.VERSION_CONFLICT,
        message: 'Replay ID was already used for another request.',
      });
    return {
      replayId: input.replayId,
      eventId: row.event_id,
      previousAttempt: p.previous_attempt,
      attemptLimit: p.next_limit,
      replayed: true,
    };
  }
  if (row.status !== 'dead')
    throw new DomainError({
      ...
      message: 'Only dead Consumer receipts may be replayed.',
    });
  if (row.attempt_count !== input.expectedAttempt)
    throw new DomainError({
      code: ErrorCode.VERSION_CONFLICT,
      message: 'Consumer attempt changed; refresh before replay.',
    });
  ...
  const inserted = singleton(
    await tx.query(
      `INSERT INTO event_consumption_replays
  ...
  await tx.query(
    `UPDATE event_consumptions SET status='pending',cycle_start_attempt=attempt_count,attempt_limit=$2,
    next_attempt_at=$3,processed_at=NULL,result_json=NULL,last_error=NULL,failure_code=NULL,notify_after=$3,updated_at=$3 WHERE id=$1`,
    [row.id, attemptLimit, at],
  );
  return {
    replayId: input.replayId,
    eventId: row.event_id,
    previousAttempt: row.attempt_count,
    attemptLimit,
    replayed: false,
  };
}
