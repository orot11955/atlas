...
test('DNS deadline returns before lookup settles and a late result never sends', { timeout: 3_000 }, async () => {
  let received = 0;
  const target = await peer(httpServer((_req, res) => { received += 1; res.end(); }));
  let release!: (value: Awaited<ReturnType<WebhookAddressResolver>>) => void;
  const pending = new Promise<Awaited<ReturnType<WebhookAddressResolver>>>((resolve) => { release = resolve; });
  const sender = new NodeWebhookSender(localOptions, () => pending);
  try {
    await assert.rejects(sender.send(request(`http://hooks.example.com:${target.port}`, 40)), rejectsWith('deadline-exceeded'));
...
test('DNS and response headers share one deadline rather than separate budgets', { timeout: 3_000 }, async () => {
  const target = await peer(httpServer((_req, res) => {
  ...
  try {
    ...
    await assert.rejects(sender.send(request(`http://hooks.example.com:${target.port}`, 200)), rejectsWith('deadline-exceeded'));
    assert.ok(performance.now() - started < 1_500);
    await target.disconnected();
  } finally { await target.close(); }
});

test('slow headers cannot retain a socket past the total deadline', { timeout: 3_000 }, async () => {
  const target = await peer(httpServer(() => {}));
  try {
    await assert.rejects(new NodeWebhookSender(localOptions).send(request(target.url, 50)), rejectsWith('deadline-exceeded'));
    await target.disconnected();
  } finally { await target.close(); }
});

test('continuously trickled response bytes do not extend the deadline', { timeout: 3_000 }, async () => {
  const target = await peer(httpServer((_req, res) => {
    res.writeHead(200);
    res.flushHeaders();
  ...
  try {
    await assert.rejects(new NodeWebhookSender(localOptions).send(request(target.url, 80)), rejectsWith('deadline-exceeded'));
    await target.disconnected();
  } finally { await target.close(); }
});
...
test('TLS handshake stalls are bounded by the same deadline', { timeout: 3_000 }, async () => {
  ...
  try {
    await assert.rejects(new NodeWebhookSender(localOptions).send(request(`https://127.0.0.1:${target.port}`, 80)), rejectsWith('deadline-exceeded'));
    await target.disconnected();
  } finally { await target.close(); }
});

for (const declared of [true, false]) {
  test(`oversized ${declared ? 'declared' : 'chunked'} response is closed without draining`, { timeout: 3_000 }, async () => {
    const target = await peer(httpServer((_req, res) => {
...
test('pinned host, signature and body survive transport without retaining the response', async () => {
  ...
  const target = await peer(httpServer((req, res) => {
    let body = '';
    req.on('data', (chunk: Buffer) => { body += chunk.toString('utf8'); });
    req.on('end', () => {
      captured = { host: req.headers.host, signature: req.headers['x-atlas-signature'] as string, body };
      res.end(secret);
    });
  }));
  const sender = new NodeWebhookSender(localOptions, async () => [{ address: '127.0.0.1', family: 4 }]);
  try {
    const input = request(`http://hooks.example.com:${target.port}/callback`);
    const response = await sender.send({ ...input, headers: { ...input.headers, Host: 'attacker.invalid', 'content-length': '0' } });
    assert.deepEqual(captured, { host: `hooks.example.com:${target.port}`, signature: input.headers['x-atlas-signature'], body: input.body });
...
test('empty response has no body diagnostic', async () => {
  const target = await peer(httpServer((_req, res) => { res.writeHead(204); res.end(); }));
  try {
    assert.deepEqual(await new NodeWebhookSender(localOptions).send(request(target.url)), { status: 204 });
  } finally { await target.close(); }
});
...
test('truncated response fails rather than acknowledging an incomplete body', { timeout: 3_000 }, async () => {
  const target = await peer(httpServer((_req, res) => {
  ...
  try {
    await assert.rejects(new NodeWebhookSender(localOptions).send(request(target.url)), rejectsWith('response-incomplete'));
  } finally { await target.close(); }
});

test('mixed public/private DNS answers and unsafe URL policies fail before connection', async () => {
  const sender = new NodeWebhookSender({ allowHttp: false, allowPrivateNetwork: false }, async () => [
    { address: '8.8.8.8', family: 4 }, { address: '10.0.0.1', family: 4 },
  ]);
  for (const url of ['https://hooks.example.com', 'http://hooks.example.com', 'https://127.0.0.1', 'https://user:password@hooks.example.com']) {
...
test('DNS failure messages never echo resolver input', async () => {
  const sender = new NodeWebhookSender(localOptions, async () => { throw new Error(secret); });
...
test('unbounded or invalid timeout and response limits are rejected', async () => {
  ...
  for (const timeout of [0, -1, 1.5, Number.NaN, Infinity, 120_001]) {
    await assert.rejects(sender.send(request('http://127.0.0.1', timeout)), rejectsWith('processing-failed'));
  }
