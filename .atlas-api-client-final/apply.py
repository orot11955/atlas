...
service_content = service_content.replace('  type ApiClientScope,\n', '')
service_content = service_content.replace('  type ApiClientType,\n', '')
service_path.write_text(service_content, encoding='utf-8')

manager_path = ROOT / 'apps/admin-web/src/features/api-clients/api-client-manager.tsx'
...
manager_path.write_text(manager_content, encoding='utf-8')

required = [
    ROOT / 'scripts/ci/admin-auth-e2e.mjs',
    ROOT / 'scripts/ci/api-client-lifecycle-e2e.mjs',
    ROOT / 'packages/server/src/api-client.test.ts',
]
missing = [str(path.relative_to(ROOT)) for path in required if not path.is_file()]
if missing:
    raise RuntimeError(f'Required API Client verification files are missing: {missing}')
