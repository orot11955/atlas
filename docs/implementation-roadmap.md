...
|     1 | Server Boundary & Platform Kernel Lite | 완료 | API·Worker 공유 코드 경계와 최소 공통 기반     |
|     2 | Admin Identity & Shell                 | 완료 | OWNER 로그인, MFA, Session, 기본 관리자 Shell  |
|     3 | Workspace, Site & API Client           | 완료 | 다중 Site와 Site별 Server-to-server 인증       |
|     4 | Project & Deployment Read Model        | 완료 | 프로젝트 이력과 CI 배포 상태 조회              |
|     5 | Resource & Member Directory MVP        | 완료 | 개인 자료와 Site별 기본 회원 관리              |
|     6 | Content Draft & Revision               | 완료 | Autosave Draft와 불변 Revision                 |
|     7 | Publication & Delivery API             | 완료 | Site별 게시와 외부 읽기 API                    |
|     8 | MinIO Media                            | 완료 | 원본 Upload, Variant와 Asset Picker            |
|     9 | Outbox, Webhook & Scheduling           | 완료 | 신뢰성 있는 비동기 처리와 예약 게시            |
|    10 | Content Operations                     | 다음 | Taxonomy, Redirect, Navigation, Feed와 Search  |
